...
def getsession(request):
    ...
    fastname = request.session.get("fastname", "default")
...
def delsession(request):
    if "fastname" in request.session:
...
def sessionmethodsinview(request):
    ...
    print("expire at_browser_close", expire_at_browser_close)
...
def sessionmethodsintemplate(request):
    ...
    items = request.session.items()
    return render(
        request, "student/sessionmethodsintemplate.html", {"keys": keys, "items": items}
    )
